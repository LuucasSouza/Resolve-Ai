...
import type { InterviewAnswers, InterviewResult, InterviewState } from "../types/runtime.js";
...
function answerFrom(lines: string[], index: number, fallback: string): string {
  const value = lines[index]?.trim();
  if (!value || /^não sei$/i.test(value) || /^nao sei$/i.test(value)) return fallback;
  return value;
}

export function buildInterviewAnswers(lines: string[] = readStdinLines()): { answers: InterviewAnswers; interactive: boolean } {
...
function resumo(answers: InterviewAnswers): string {
...
function front(title: string, generatedAt: string): string {
  return `---\ntitle: "${title}"\ngenerated_by: "Resolve Aí"\ncommand: "resolve-ai entrevistar"\ngenerated_at: "${generatedAt}"\nstatus: "draft"\n---\n\n`;
}

function intakeContent(answers: InterviewAnswers, generatedAt: string): string {
...
function discoveryContent(answers: InterviewAnswers, generatedAt: string): string {
...
function productContent(answers: InterviewAnswers, generatedAt: string): string {
...
function isTemplateLike(content: string): boolean {
...
function writeOrAppend(target: string, label: string, content: string, generatedAt: string): { created: string[]; updated: string[]; preserved: string[] } {
...
export function runGuidedInterview(root: string = process.cwd()): InterviewResult {
...
export function interviewQuestionsText(): string {
